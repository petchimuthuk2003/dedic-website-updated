...
const PaymentPage: React.FC = () => {
    ...
    const handlePayment = async (e: React.FormEvent) => {
        ...
        const options = {
            ...
            amount: '99900', // Amount is in currency subunits. Default currency is INR. Hence, 99900 refers to 999 INR
            currency: 'INR',
            name: 'Dedic Infotech',
            description: 'UI/UX Design Master Course',
            image: '/Public/uiux-course-thumbnail.webp',
            handler: function (response: any) {
                alert(`Payment Successful! Payment ID: ${response.razorpay_payment_id}`);
                // navigate('/dashboard'); // Or navigate to a success page
            },
            prefill: {
        ...
        <div className="min-h-screen flex flex-col selection:bg-tech-blue/20 selection:text-tech-blue pt-32 pb-12">
            ...
            <main className="flex-grow z-10 px-4 md:px-6">
                <div className="container mx-auto max-w-6xl">
                    <button 
                        onClick={() => navigate(-1)} 
                        className="flex items-center gap-2 text-slate-500 hover:text-tech-blue font-semibold mb-8 transition-colors group"
                    >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
                        ...
                        <div className="lg:col-span-7 space-y-8">
                            <div>
                                <h1 className="text-3xl md:text-5xl font-extrabold text-app-slate tracking-tighter mb-4">
                                    Secure Checkout
                                </h1>
                                <p className="text-slate-500 font-medium text-lg">
                                    Please enter your billing details to complete the purchase.
                                </p>
                            </div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-2">
                                            ...
                                            <input id="fullname" required type="text" placeholder="e.g. John Doe" className="w-full px-5 py-4 bg-white/80 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-tech-blue/10 focus:border-tech-blue transition-all font-medium text-slate-800 placeholder:text-slate-400" />
                                        </div>
                                        <div className="space-y-2">
                                            <label htmlFor="phone" className="text-sm font-bold text-slate-700">Phone Number</label>
                                            <input id="phone" required type="tel" placeholder="+91 98765 43210" className="w-full px-5 py-4 bg-white/80 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-tech-blue/10 focus:border-tech-blue transition-all font-medium text-slate-800 placeholder:text-slate-400" />
                                        </div>
                                    </div>

                                    <div className="space-y-2">
                                        <label htmlFor="email" className="text-sm font-bold text-slate-700">Email Address</label>
                                        <input id="email" required type="email" placeholder="john@example.com" className="w-full px-5 py-4 bg-white/80 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-tech-blue/10 focus:border-tech-blue transition-all font-medium text-slate-800 placeholder:text-slate-400" />
                                    </div>
                                    
                                    <div className="space-y-2">
                                        <label htmlFor="address" className="text-sm font-bold text-slate-700">Address (Optional)</label>
                                        <textarea id="address" rows={3} placeholder="Your address details..." className="w-full px-5 py-4 bg-white/80 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-tech-blue/10 focus:border-tech-blue transition-all resize-none font-medium text-slate-800 placeholder:text-slate-400"></textarea>
                                    </div>
                                </div>
                            </form>
                        </div>
                        ...
                        <div className="lg:col-span-5">
                            <div className="glass-panel p-6 md:p-8 rounded-[2rem] border-slate-200 shadow-2xl sticky top-32 space-y-8">
                                <div>
                                    ...
                                    <div className="flex gap-5 items-center bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-app-slate leading-snug text-lg">UI/UX Design Master Course with AI</h4>
                                ...
                                <div className="pt-6 border-t border-slate-100">
                                    <div className="flex justify-between items-end mb-8">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <span className="text-5xl font-black text-tech-blue tracking-tighter">₹999</span>
                                        </div>
                                    </div>
                                    ...
                                    <button 
                                        ...
                                        form="checkout-form"
                                        disabled={isLoading}
                                        className="w-full py-5 bg-tech-blue hover:bg-blue-700 text-white font-black rounded-2xl transition-all shadow-[0_10px_40px_-10px_rgba(37,99,235,0.5)] hover:shadow-[0_15px_50px_-10px_rgba(37,99,235,0.6)] text-xl flex items-center justify-center gap-3 group disabled:opacity-70 disabled:cursor-not-allowed transform hover:-translate-y-1"
                                    >
                                        {isLoading ? (
                                            ...
                                            <>
                                                Pay ₹999 Now
                                                <CheckCircle2 size={24} className="group-hover:scale-110 group-hover:rotate-12 transition-transform" />
                                            </>
                                        )}
                                    </button>
                                </div>
